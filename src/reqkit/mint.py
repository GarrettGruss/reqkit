...
def mint_requirements(
    ...
    return [
        mint_requirement(subtype, category, parent_id, parent_rel) for _ in range(qty)
    ]
...
def mint_requirement(
    ...
    return ReqkitRequirement(
        subtype=validate_requirement_type(subtype) if subtype is not None else None,
        ...
        parent_rel=validate_relationship_type(parent_rel)
